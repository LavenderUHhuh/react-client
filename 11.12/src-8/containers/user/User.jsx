...
import {getUsers,addUser,updateUser} from '../../redux/ation-creators'
@connect(state=>({users:state.users,username:state.user.username}),{getUsers,addUser,updateUser})
...
componentDidMount(){
  ...
  // 创建用户的回调函数
  addUser = () => {
    this.addUserFormRef.validateFields((err,values)=>{
      if(!err){
        const {name}=values
